...
static int
year (TM *tm, const int *digit_pair, size_t n, unsigned int syntax_bits)
{
  switch (n)
    {
    case 1:
	...
	/* Use current year.  */
	if (! currentft (&now) || ! ft2sec (&now, &seconds, &nsec)
	    || ! localtimew (&seconds, &tmp))
	  return 1;
...
static int
posix_time_parse (TM *tm, const char *s, unsigned int syntax_bits)
{
  const char *dot = NULL;
  int pair[6];
  int *p;
...
bool
posixtime (intmax_t *p, const char *s, unsigned int syntax_bits)
{
  TM tm0
#ifdef lint
